  ...
  if (!budgets.length) {
    return (
      <div className="glass-card p-5">
        <h3 className="font-display font-semibold text-foreground text-sm mb-4">{tx.budgetsThisMonth}</h3>
        <EmptyState icon={PieChart} title={tx.noBudgets} description={tx.noBudgetsDesc} />
      </div>
    );
  }
  ...
  return (
    <div className="glass-card p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display font-semibold text-foreground text-sm">{tx.budgetsThisMonth}</h3>
        <Link href="/budgets" className="text-xs text-primary hover:underline font-medium">{tx.manageBudgets}</Link>
      </div>
            ...
            <div key={budget.id} className="space-y-1.5">
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: budget.category?.color ?? "#10b981" }} />
